...
def crear_db():
    ...
    cursor.execute(
        """CREATE TABLE IF NOT EXISTS alumnos(id INTEGER PRIMARY KEY AUTOINCREMENT,
                       nombre VARCHAR(30),
                       apellido VARCHAR(30),
                       materia VARCHAR(30),
...
def login():
    if request.method == 'POST':
        ...
        if any(entry[1] == usuario and entry[2] == passw
               for entry in log) and (usuario, passw) is not None:
